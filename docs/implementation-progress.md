...
## Terse-By-Default Write/Aggregate Responses On 2026-05-31 (#4)

- Writes and `aggregate pages` no longer echo the full Notion API payload by default, which was expensive noise inside an agent context window.
- `block append` now returns `{page_id, appended_count, block_ids}`; `page create` and `page properties update` return the normalized page (plus the create reminder) without the request-body echo; `aggregate pages` groups rows under a per-database `databases` array (database identity named once instead of repeated per record); within each database rows are bucketed by their live Notion status value (`by_status` keyed by e.g. `In progress`/`Done`, replacing the earlier synthetic `commitments`/`completed_work`/`planning_candidates`/`stale_work` labels). Each page is a `{id, title, last_edited, agent_notes}` summary instead of full normalized property JSON: the URL is dropped (inferable from the id), `last_edited` is date-only, and `status` is omitted because rows are already grouped under it. `--databases <id|title,...>` scopes the roll-up to a subset of approved databases (failing closed on anything outside the Gateway registry). By default the command hides completed work (status options named done/complete/completed) by querying only the active status options per database, and emits a `status_hint`; `--all` removes the status filter entirely and an explicit `--status` overrides the default. This keeps the default working view focused on WIP and not-started tasks. `agent_notes` is included only when the page's database carries an Agent Notes property; `last_edited_time` is always present so agents keep the highest-level recency context.
- Terse responses carry a `hint` field pointing at `--verbose`, so the discoverability lives in the CLI output itself rather than in the skill doc.
- A new global `--verbose` flag (alias `--format full`) restores the full echo: the SDK request body / `plan` on writes, the raw `response` on `block append`, and full normalized pages on `aggregate pages`. Dry-run plans are unchanged because previewing the request is their purpose.
- `aggregate pages` is now bounded for context protection: previously a bare `aggregate pages` paginated every row of every approved database into the response. It now caps results at 10 per database by default (`--limit N` to override), queries most-recently-edited first so the truncated sample is the freshest, reports per-database `truncated`, and adds a top-level `truncated` + actionable `note` when rows were dropped. The bounded fetch pulls a single page of `limit+1` rows instead of paginating the whole database.
